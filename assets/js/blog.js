...
const retrieve = () => {
	fetch('https://api.rss2json.com/v1/api.json?api_key=4iwmzf0yryfd6ao88oqtqao8cxrwxrdefcutllj4&rss_url=https://medium.com/feed/@cunycrypto')
	.then(res => {
		if (res.ok) {
			return res.json();
...
const blogDisp = posts => {
	...
	for (let i = 0; i < posts.items.length; i++) {
		...
		date.innerHTML = `<p>By: <a href="https://medium.com/@cunycrypto">${posts.items[i].author}</a> <br> ${posts.items[i].pubDate.substr(0,posts.items[i].pubDate.indexOf(' '))}</p>`;
